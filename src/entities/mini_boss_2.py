# Biome 2 mini boss (96×96). Same states as mini_boss_1; uses mini_boss_2 assets.
...
MINI_BOSS_2_STOP_DISTANCE = 70.0  # px; attack range for melee


class MiniBoss2:
    """
    Biome 2 mini boss. 96×96, confined to room. Uses mini_boss_2 assets.
    ...
    def _ensure_animations_loaded(self) -> None:
        ...
        base_path = "assets/entities/enemies/mini_boss_2"
        idle_frames: list[pygame.Surface] = []
        for state in MINI_BOSS_2_STATES:
    ...
    def update(self, dt: float, player, room_rect: pygame.Rect | None = None) -> None:
        ...
        if dist > MINI_BOSS_2_STOP_DISTANCE:
            if dist > 1e-3:
                nx = dx / dist
                ny = dy / dist
