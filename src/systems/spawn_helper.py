...
# Slow tiles allowed; lava excluded so enemies never spawn standing in lava (visual + pathing).

import math
import random
from typing import List, Set, Tuple

from dungeon.room import TILE_FLOOR, TILE_SLOW
from game.config import (
    ...
    SPAWN_DEBUG_LOG,
    DEBUG_HEAVY_SPAWN,
    SPAWN_EXTRA_EDGE_PADDING_TILES,
    SPAWN_EXTRA_EDGE_PADDING_TILES_HEAVY,
    SPAWN_EXTRA_TOP_ROWS_TILES,
    SPAWN_RANDOM_VALIDATION_ATTEMPTS,
)
from systems.collisions import tile_range_for_centered_aabb


# Walkable tile types for enemy spawn (floor + slow; lava excluded — not a valid spawn stand).
_VALID_SPAWN_TILE_TYPES = (TILE_FLOOR, TILE_SLOW)
...
def _is_tile_valid_for_spawn(room, col: int, row: int) -> bool:
    """True if (col, row) is inside playable bounds, not a wall, and not a decorative obstacle. Floor/slow allowed."""
...
def _spawn_padded_inner_tile_bounds(room, for_heavy: bool) -> Tuple[int, int, int, int]:
    """
    Inner tile range for random spawns: wall margin + extra edge padding (Heavy: wider),
    plus extra top rows to avoid buggy top wall rows.
    """
    ...
    wall_margin = SPAWN_HEAVY_WALL_TILE_MARGIN if for_heavy else MIN_TILES_FROM_WALL + SPAWN_EXTRA_INTERIOR_WALL_TILES
    edge_pad = SPAWN_EXTRA_EDGE_PADDING_TILES_HEAVY if for_heavy else SPAWN_EXTRA_EDGE_PADDING_TILES
    inner_min_row = playable_min_row + wall_margin + SPAWN_EXTRA_TOP_ROWS_TILES
    inner_max_row = playable_max_row - wall_margin - edge_pad
    inner_min_col = playable_min_col + wall_margin + edge_pad
    inner_max_col = playable_max_col - wall_margin - edge_pad
...
def _spawn_heavy_debug_position(room, world_x: float, world_y: float) -> None:
    """Quick visibility: Heavy spawn location vs top-edge issues (enable DEBUG_HEAVY_SPAWN)."""
    if not DEBUG_HEAVY_SPAWN:
        return
    print(f"[SPAWN] Heavy at ({world_x:.1f},{world_y:.1f}) room={_room_debug_id(room)}")
...
def _first_valid_spawn_row_major(
    ...
    inner_min_row, inner_max_row, inner_min_col, inner_max_col = _spawn_padded_inner_tile_bounds(
        room, for_heavy
    )
...
def _heavy_spawn_inflated_clearance_ok(
    room,
    wx: float,
    wy: float,
    blocked_tiles: Set[Tuple[int, int]] | None,
) -> bool:
    """Match GameScene._heavy_clearance_ok: movement hitbox inflated by 2×HEAVY_CLEARANCE_PADDING_PX must not overlap walls or solid props."""
    mw, mh = enemy_movement_size_tuple("heavy")
    pad = float(HEAVY_CLEARANCE_PADDING_PX)
    full_w = float(mw) + 2.0 * pad
    full_h = float(mh) + 2.0 * pad
    min_tx, max_tx, min_ty, max_ty = tile_range_for_centered_aabb(wx, wy, full_w, full_h)
...
def _reject_reason_heavy_spawn(
    ...
    if _hard_corner_zone(room, col, row, cz):
        return "corner"
    b = room.wall_border()
    hm = SPAWN_HEAVY_WALL_TILE_MARGIN
    if col < b + hm or col >= room.width - b - hm:
        return "edge_margin"
    if row < b + hm or row >= room.height - b - hm:
        return "edge_margin"
    if blocked_tiles:
    ...
    if not _heavy_spawn_inflated_clearance_ok(room, float(tcx), float(twy), blocked_tiles):
        return "heavy_clearance"
    return None
...
def _candidate_passes_spawn_checks(
    room,
    col: int,
    row: int,
    x: float,
    ...
    """Walkability + distance from player + doors + overlap + final world validation."""
    if for_heavy:
        if _reject_reason_heavy_spawn(room, col, row, blocked_tiles) is not None:
            return False
    else:
        if _reject_reason_standard_spawn(room, col, row) is not None:
            return False
    px, py = player_center_xy
    if math.hypot(x - px, y - py) < MIN_DISTANCE_FROM_PLAYER_PX:
        return False
    for (dx, dy) in door_positions:
        if math.hypot(x - dx, y - dy) < MIN_TILES_FROM_DOOR * TILE_SIZE:
            return False
    if _position_overlaps_existing(x, y, existing_positions, spacing):
        return False
    et = _effective_spawn_enemy_type(for_heavy, enemy_type)
    min_x, min_y, max_x, max_y = _spawn_playable_bounds(room, et)
    x2 = max(min_x, min(max_x, x))
    y2 = max(min_y, min(max_y, y))
    blocked = blocked_tiles if for_heavy else None
    fv = _validate_final_spawn_world(room, x2, y2, for_heavy, blocked, enemy_type=enemy_type)
    return fv is None
...
def generate_valid_spawn_position(
    ...
    Respects: MIN_DISTANCE_FROM_PLAYER, MIN_TILES_FROM_WALL (or SPAWN_HEAVY_WALL_TILE_MARGIN if for_heavy),
    MIN_TILES_FROM_DOOR, MIN_DISTANCE_BETWEEN_ENEMIES (plus ELITE_EXTRA_SPACING_PX if is_elite).
    When for_heavy=True, also excludes corner zones (MIN_TILES_FROM_CORNER_HEAVY) and requires inflated Heavy clearance (movement + clearance padding).
    ...
    for _ in range(SPAWN_RANDOM_VALIDATION_ATTEMPTS):
        ...
        _spawn_heavy_debug_position(room, x, y)
        return (x, y)
