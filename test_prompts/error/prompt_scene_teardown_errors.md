## Prompt Title: Scene teardown — error paths
...
### OBJECTIVE ###
Teardown during an error state (partial init, mid-transition) must not double-free or crash.

---

### CONTEXT — EXISTING GAPS ###
Multiple `Scene teardown` / `Scene` skips.

---

### FILES TO CREATE OR EXTEND ###
- `tests/error/test_scene_teardown_errors.py`
...
### TEST CASES ###
- `test_teardown_partial_init_does_not_crash`
- `test_teardown_during_transition_does_not_double_free`
- `test_teardown_with_pending_deferred_ops_drains_safely`
