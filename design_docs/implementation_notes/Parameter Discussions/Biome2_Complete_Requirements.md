# Biome 2 - Complete Requirements Breakdown (Authoritative)

This document consolidates what is currently expected to be implemented for Biome 2 (MVP + current fixes). **It is intended for Cursor AI to follow exactly.**
...
- **TITLE_SIZE_PX = 32**
- All title-based placement must align to the 32px grid.
- Walls/doors must align to title edges.
...
- The playable room is defined by this **rows x cols** grid (not a smaller "box inside the screen").
...
- **Standard rooms** (Combats, Safe): wall thickness **B = 2 tiles**
...
### 2.1 Wall tile assets (32x32 only)
...
### 3.1 Door assets (32x32 sprites scaled)
...
## 5) Combat & Hit Detection (Applies to ALL enemies)
...
## 11) Debugging Aids (Must be available)
...
- [ ] Door opens only after enemy clear + 2.5s delay 
